...
string Irq2Operation::toString() {
    ...
    oinf += " RisingCommand:'" + cmd1 + "' FallingCommand:'" + cmd2 + "' DebounceMS:" + to_string(debounceMS);
...
bool Irq2Operation::execute(AppInfo * appInfo) {
    ...
    if (pid == 0) {
        ...
        oinf += " RisingCommand:'" + cmd1ex + "' FallingCommand:'" + cmd2ex + "' DebounceMS:" + to_string(debounceMS);
        ...
        pin->setIrq(GPIO_IRQ_BOTH, irq2HandlerObj, debounceMS);
...
string Irq2Operation::help() {
    string hStr;
    hStr = "irq2 <pin-number> <rising-command> <falling-command> <optional-debounce>";
    hStr = hStr + "\n\tStarts a separate process to respond to interrupts";
    hStr = hStr + "\n\ton given <pin-number> and perform given commands on interrupt";
